...
coor_data = pandas.read_csv('training\\train_coor_data.csv')
coor_data = coor_data.values

face_crop = face_crop_level2(train_data_x, coor_data)
left_eye_data, left_eye_top_left = face_crop.left_eye_crop()
left_eye_data = numpy.array(left_eye_data)
right_eye_data, right_eye_top_left = face_crop.right_eye_crop()
right_eye_data = numpy.array(right_eye_data)
nose_data, nose_top_left = face_crop.nose_crop()
nose_data = numpy.array(nose_data)
left_mouth_data, left_mouth_top_left = face_crop.left_mouth_crop()
left_mouth_data = numpy.array(left_mouth_data)
right_mouth_data, right_mouth_top_left = face_crop.right_mouth_crop()
...
left_eye_coor = left_eye_coor - left_mouth_top_left
...
left_eye_cost_list = []
left_coor_level2 = numpy.zeros((left_eye_coor.shape[0], left_eye_coor.shape[1]))
    ...
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                left_coor_level2[start:, :] = sess.run(left_eye_layer4, feed_dict=feed_dict)
            else:
                input_x = left_eye_data[start: start + per_train_num, :, :, :]
                input_y = left_eye_coor[start: start + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                left_coor_level2[start: start + per_train_num, :] = sess.run(left_eye_layer4, feed_dict=feed_dict)
            _, cost = sess.run([left_eye_train_op, left_eye_cost], feed_dict=feed_dict)
            left_eye_cost_list.append(cost)
    left_coor_level2 = left_coor_level2 + left_eye_top_left
    save_path = saver.save(sess, 'save/left_eye_model_level2_1.ckpt')
    plt.plot(range(len(left_eye_cost_list)), left_eye_cost_list)
    plt.title('left_eye_train_cost_level2_1')
    plt.show()
sess.close()
...
saver = tf.train.Saver()
right_eye_cost_list = []
right_coor_level2 = numpy.zeros((right_eye_coor.shape[0], right_eye_coor.shzpe[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                right_coor_level2[start:, :] = sess.run(right_eye_layer4, feed_dict=feed_dict)
            else:
                input_x = right_eye_data[start: start + per_train_num, :, :, :]
                input_y = right_eye_coor[start: start + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                right_coor_level2[start: start + per_train_num, :] = sess.run(right_eye_layer4, feed_dict=feed_dict)
            _, cost = sess.run([right_eye_train_op, right_eye_cost], feed_dict=feed_dict)
            right_eye_cost_list.append(cost)
    right_coor_level2 = right_coor_level2 + right_eye_top_left
    save_path = saver.save(sess, 'save/right_eye_model_level2_1.ckpt')
    plt.plot(range(len(right_eye_cost_list)), right_eye_cost_list)
    plt.title('right_eye_train_cost_levle2_1')
    plt.show()
sess.close()


# nose NN1
nose_tip_coor = train_data_y.loc[:, ['nose_tip_x', 'nose_tip_y']]
nose_tip_coor = nose_tip_coor.values
nose_tip_coor = nose_tip_coor - nose_top_left
x = tf.placeholder(shape=[None, nose_data.shape[1], nose_data.shape[2], 1], dtype=tf.float32)
y = tf.placeholder(shape=[None, nose_tip_coor.shape[1]], dtype=tf.float32)
nose_train_op, nose_layer4, nose_cost = build_keypoint_detection(nose_data, nose_tip_coor, x, y)
per_train_num = nose_tip_coor.shape[0] // 100
saver = tf.train.Saver()
nose_cost_list = []
nose_coor_level2 = numpy.zeros((nose_tip_coor.shape[0], nose_tip_coor.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                input_y = nose_tip_coor[start:, :]
                feed_dict = {x: input_x, y: input_y}
                nose_coor_level2[start:, :] = sess.run(nose_layer4, feed_dict=feed_dict)
            else:
                input_x = nose_data[start: start + per_train_num, :, :, :]
                input_y = nose_tip_coor[start: start + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                nose_coor_level2[start: start + per_train_num, :] = sess.run(nose_layer4, feed_dict=feed_dict)
            _, cost = sess.run([nose_train_op, nose_cost], feed_dict=feed_dict)
            nose_cost_list.append(cost)
    nose_coor_level2 = nose_coor_level2 + nose_top_left
    save_path = saver.save(sess, 'save/nose_model_level2_1.ckpt')
    plt.plot(range(len(nose_cost_list)), nose_cost_list)
    plt.title('nose_train_cost_levle2_1')
    plt.show()
sess.close()
...
left_mouth_coor_level2 = numpy.zeros((left_mouth_coor.shape[0], left_mouth_coor.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                left_mouth_coor_level2[start:, :] = sess.run(left_mouth_layer4, feed_dict=feed_dict)
            else:
                input_x = left_mouth_data[start: start + per_train_num, :, :, :]
                input_y = left_mouth_coor[start: start + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                left_mouth_coor_level2[start: start + per_train_num, :] = sess.run(left_mouth_layer4, feed_dict=feed_dict)
            _, cost = sess.run([left_mouth_train_op, left_mouth_cost], feed_dict=feed_dict)
            left_mouth_cost_list.append(cost)
    left_mouth_coor_level2 = left_mouth_coor_level2 + left_mouth_top_left
    save_path = saver.save(sess, 'save/left_mouth_model_level2_1.ckpt')
    plt.plot(range(len(left_mouth_cost_list)), left_mouth_cost_list)
    plt.title('left_mouth_train_cost_levle2_1')
    plt.show()
sess.close()


# right mouth NN1
right_mouth_coor = train_data_y.loc[:, ['mouth_left_corner_x', 'mouth_left_corner_y']]
...
right_mouth_coor_level2 = numpy.zeros((right_mouth_coor.shape[0], right_mouth_coor.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                right_mouth_coor_level2[start:, :] = sess.run(right_mouth_layer4, feed_dict=feed_dict)
            else:
                input_x = right_mouth_data[start: start + per_train_num, :, :, :]
                input_y = right_mouth_coor[start: start + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                right_mouth_coor_level2[start: start + per_train_num, :] = sess.run(right_mouth_layer4, feed_dict=feed_dict)
            _, cost = sess.run([right_mouth_train_op, right_mouth_cost], feed_dict=feed_dict)
            right_mouth_cost_list.append(cost)
    right_mouth_coor = right_mouth_coor + right_mouth_top_left
    save_path = saver.save(sess, 'save/right_mouth_model_level2_1.ckpt')
    plt.plot(range(len(right_mouth_cost_list)), right_mouth_cost_list)
    plt.title('right_mouth_train_cost_levle2_1')
    plt.show()
sess.close()
...
left_coor_level2_1 = numpy.zeros((left_eye_coor_1.shape[0], left_eye_coor_1.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(1, 100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                left_coor_level2_1[: index, :] = sess.run(left_eye_layer4_1, feed_dict=feed_dict)
            else:
                input_x = left_eye_data[index: index + per_train_num, :, :, :]
                input_y = left_eye_coor_1[index: index + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                left_coor_level2_1[index: index + per_train_num, :] = sess.run(left_eye_layer4_1, feed_dict=feed_dict)
            _, cost = sess.run([left_eye_train_op_1, left_eye_cost_1], feed_dict=feed_dict)
            left_eye_cost_list_1.append(cost)
    left_coor_level2_1 = left_coor_level2_1 + left_eye_top_left
    save_path = saver.save(sess, 'save/left_eye_model_level2_2.ckpt')
    plt.plot(range(len(left_eye_cost_list_1)), left_eye_cost_list_1)
    plt.title('left_eye_train_cost_levle2_2')
    plt.show()
sess.close()
...
right_coor_level2_1 = numpy.zeros((right_eye_coor_1.shape[0], right_eye_coor_1.shzpe[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(1, 100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                right_coor_level2_1[: index, :] = sess.run(right_eye_layer4_1, feed_dict=feed_dict)
            else:
                input_x = right_eye_data[index: index + per_train_num, :, :, :]
                input_y = right_eye_coor_1[index: index + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                right_coor_level2_1[index: index + per_train_num, :] = sess.run(right_eye_layer4_1, feed_dict=feed_dict)
            _, cost = sess.run([right_eye_train_op_1, right_eye_cost_1], feed_dict=feed_dict)
            right_eye_cost_list_1.append(cost)
    right_coor_level2_1 = right_coor_level2_1 + right_eye_top_left
    save_path = saver.save(sess, 'save/right_eye_model_level2_2.ckpt')
    plt.plot(range(len(right_eye_cost_list_1)), right_eye_cost_list_1)
    plt.title('right_eye_train_cost_levle2_2')
    plt.show()
sess.close()


# nose NN2
nose_tip_coor_1 = train_data_y.loc[:, ['nose_tip_x', 'nose_tip_y']]
nose_tip_coor_1 = nose_tip_coor_1.values
nose_tip_coor_1 = nose_tip_coor_1 - nose_top_left
x = tf.placeholder(shape=[None, nose_data.shape[1], nose_data.shape[2], 1], dtype=tf.float32)
y = tf.placeholder(shape=[None, nose_tip_coor_1.shape[1]], dtype=tf.float32)
nose_train_op_1, nose_layer4_1, nose_cost_1 = build_keypoint_detection(nose_data, nose_tip_coor_1, x, y)
per_train_num = nose_tip_coor_1.shape[0] // 100
saver = tf.train.Saver()
nose_cost_list_1 = []
nose_coor_level2_1 = numpy.zeros((nose_tip_coor_1.shape[0], nose_tip_coor_1.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                input_y = nose_tip_coor_1[: index, :]
                feed_dict = {x: input_x, y: input_y}
                nose_coor_level2_1[: index, :] = sess.run(nose_layer4_1, feed_dict=feed_dict)
            else:
                input_x = nose_data[index: index + per_train_num, :, :, :]
                input_y = nose_tip_coor_1[index: index + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                nose_coor_level2_1[index: index + per_train_num, :] = sess.run(nose_layer4_1, feed_dict=feed_dict)
            _, cost = sess.run([nose_train_op_1, nose_cost_1], feed_dict=feed_dict)
            nose_cost_list_1.append(cost)
    nose_coor_level2_1 = nose_coor_level2_1 + nose_top_left
    save_path = saver.save(sess, 'save/nose_model_level2_2.ckpt')
    plt.plot(range(len(nose_cost_list_1)), nose_cost_list_1)
    plt.title('nose_train_cost_levle2_2')
    plt.show()
sess.close()
...
left_mouth_coor_level2_1 = numpy.zeros((left_mouth_coor_1.shape[0], left_mouth_coor_1.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                left_mouth_coor_level2_1[: index, :] = sess.run(left_mouth_layer4_1, feed_dict=feed_dict)
            else:
                input_x = left_mouth_data[index: index + per_train_num, :, :, :]
                input_y = left_mouth_coor_1[index: index + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                left_mouth_coor_level2_1[index: index + per_train_num, :] = sess.run(left_mouth_layer4_1, feed_dict=feed_dict)
            _, cost = sess.run([left_mouth_train_op_1, left_mouth_cost_1], feed_dict=feed_dict)
            left_mouth_cost_list_1.append(cost)
    left_mouth_coor_level2_1 = left_mouth_coor_level2_1 + left_mouth_top_left
    save_path = saver.save(sess, 'save/left_mouth_model_level2_2.ckpt')
    plt.plot(range(len(left_mouth_cost_list_1)), left_mouth_cost_list_1)
    plt.title('left_mouth_train_cost_levle2_2')
    plt.show()
sess.close()


# right mouth NN2
right_mouth_coor_1 = train_data_y.loc[:, ['mouth_left_corner_x', 'mouth_left_corner_y']]
...
right_mouth_coor_level2_1 = numpy.zeros((right_mouth_coor_1.shape[0], right_mouth_coor_1.shape[1]))
init = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init)
    for i in range(20):
        for j in range(100):
            ...
            if(j == 99):
                ...
                feed_dict = {x: input_x, y: input_y}
                right_mouth_coor_level2_1[: index, :] = sess.run(right_mouth_layer4_1, feed_dict=feed_dict)
            else:
                input_x = right_mouth_data[index: index + per_train_num, :, :, :]
                input_y = right_mouth_coor_1[index: index + per_train_num, :]
                feed_dict = {x: input_x, y: input_y}
                right_mouth_coor_level2_1[index: index + per_train_num, :] = sess.run(right_mouth_layer4_1, feed_dict=feed_dict)
            _, cost = sess.run([right_mouth_train_op_1, right_mouth_cost_1], feed_dict=feed_dict)
            right_mouth_cost_list_1.append(cost)
    right_mouth_coor_level2_1 = right_mouth_coor_level2_1 + right_mouth_top_left
    save_path = saver.save(sess, 'save/right_mouth_model_level2_2.ckpt')
    plt.plot(range(len(right_mouth_cost_list_1)), right_mouth_cost_list_1)
    plt.title('right_mouth_train_cost_levle2_2')
    plt.show()
sess.close()


coor_data_level2 = numpy.array((train_data_y.shape[0], train_data_y.shape[1]))
coor_data_level2[:, 0] = (left_coor_level2[:, 0] + left_coor_level2_1[:, 0]) / 2
coor_data_level2[:, 1] = (left_coor_level2[:, 1] + left_coor_level2_1[:, 1]) / 2
coor_data_level2[:, 2] = (right_coor_level2[:, 0] + right_coor_level2_1[:, 0]) / 2
coor_data_level2[:, 3] = (right_coor_level2[:, 1] + right_coor_level2_1[:, 1]) / 2
coor_data_level2[:, 4] = (nose_coor_level2[:, 0] + nose_coor_level2_1[:, 0]) / 2
coor_data_level2[:, 5] = (nose_coor_level2[:, 1] + nose_coor_level2_1[:, 1]) / 2
coor_data_level2[:, 6] = (left_mouth_coor_level2[:, 0] + left_mouth_coor_level2_1[:, 0]) / 2
coor_data_level2[:, 7] = (left_mouth_coor_level2[:, 1] + left_mouth_coor_level2_1[:, 1]) / 2
coor_data_level2[:, 8] = (right_mouth_coor_level2[:, 0] + right_mouth_coor_level2_1[:, 0]) / 2
coor_data_level2[:, 9] = (right_mouth_coor_level2[:, 1] + right_mouth_coor_level2_1[:, 1]) / 2
coor_data_level2 = pandas.DataFrame(coor_data_level2)
coor_data_level2.to_csv('coor_data_level2.csv')
